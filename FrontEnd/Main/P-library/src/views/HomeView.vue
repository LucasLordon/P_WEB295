...
<template>
  <div>
    ...
    <div v-else-if="(book !== null) && (books !== null) &&(author !== null)">
      <div>
        <title2></title2>
        <div v-if="countBook < maxlimit">
          <button @click="incrementCount">Voir le livre n° {{ countBook + 1 }}</button>
        </div>
        <p>Vous êtes au livre n° {{ countBook }}</p>
        <div v-if="countBook > limit">
          <button @click="decrementCount">Voir le livre n° {{ countBook - 1 }}</button>
        </div>
      </div>
